...
export default function PythonToolkitManager() {
  ...
  const getExecutionStatus = () => {
    if (millenniumMutation.isPending || eliteMutation.isPending) {
      return { icon: Clock, color: "text-yellow-500", text: "EXECUTING" };
    }
    if (millenniumMutation.isSuccess || eliteMutation.isSuccess) {
      return { icon: CheckCircle, color: "text-green-500", text: "SUCCESS" };
    }
    if (millenniumMutation.isError || eliteMutation.isError) {
