...
export default function VisitorTracking({ visitors, isActive }: VisitorTrackingProps) {
  ...
  const getStatusBadge = (visitor: Visitor) => {
    ...
    const isVisitorActive = timeDiff < 5 * 60 * 1000; // 5 minutes

    return (
      <Badge className={`${isVisitorActive ? 'bg-matrix/20 text-matrix' : 'bg-warning/20 text-warning'} text-xs font-mono`}>
        {isVisitorActive ? 'ACTIVE' : 'IDLE'}
      </Badge>
    );
  };
